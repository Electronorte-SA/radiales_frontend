...
export default function App() {
  ...
  const Tab = createBottomTabNavigator();
    const [inputID, setInputID] = useState('4209763');
...
//     tx.executeSql("SELECT * FROM radialesQR WHERE id = ?", [inputID], (_, { rows }) => {
//       if (rows.length > 0) {
...
function getRadialById(inputID){
...
//     let id = await getRadialById(inputID);
//     console.log('id', id)
//   }catch(ex){
...
async function handleSearchButton() {
  try {
    const id = await getRadialById(inputID);
    if (id.id !== "No encontrado") {
      
      //navigation.navigate("DatosEncontrados", { scannedDataID: id });
    } else {
      navigation.navigate("DatosNoEncontrados");
    }
  } catch (error) {
...
/////////////////////////////////////
  ...
  const handleBarCodeScanned = ({ type, data }) => {
    ...
    db.transaction((tx) => {
      tx.executeSql("SELECT * FROM radialesQR", [], (_, { rows }) => {
  ...
  //   // Aquí debes implementar la lógica para buscar la información según el ID
  //   console.log('Buscar información con el ID:', inputID);
  //   // Realiza las operaciones necesarias para buscar la información
  // };
  return (
  
<NavigationContainer>
  <Tab.Navigator>
    <Tab.Screen name="ScanearQR" component={HomeScreen} />
    {/* <Tab.Screen name="ScanData" component={ScanDataScreen} /> */}
    <Tab.Screen name="codigoQR" component={Codigo} />
        ...
        {formData.Alias ? (
          <>
            ...
            <Text style={{ textAlign: "left" }}>
              Tipo de red: {formData.Tipo_de_red}
            </Text>
            <Text style={{ textAlign: "left" }}>
              T_Nominal: {formData.T_Nominal}
            </Text>
            <Text style={{ textAlign: "left" }}>Alias: {formData.Alias}</Text>
            <Text style={{ textAlign: "left" }}>
              Nombre del propietario: {formData.Nombre_del_propietario}
            </Text>
            <Text style={{ textAlign: "left" }}>
              Estilo de subcódigo: {formData.Estilo_de_subcodigo}
            </Text>
            {formData.Latitud && (
              <Text style={{ textAlign: "left" }}>
  ...
  function Codigo({ navigation }){
    ...
    return (
      ...
      <TextInput
        ...
        onChangeText={text => setInputID(text)} // Actualiza el estado inputID al escribir
        value={inputID}
      />
      <Button title="Buscar" onPress={handleSearchButton} />
      {inputID && <ScanDataScreen/>}
    </View>
    );
  }
  // ScanDataScreen iddddddddddddddddddddddddddddddddddd con el código para mostrar los datos de la búsqueda
  function DatosEncontrados({ route }) {
    const { scannedDataID } = route.params || {};
    ...
    return (
      <View style={styles.container}>
        <Text style={styles.title}>Datos Encontrados</Text>
  
        {scannedDataID && scannedDataID.id !== "No encontrado" ? (
          // Mostrar los datos si se encuentran
          <>
            <Text>ID: {scannedDataID.id}</Text>
            <Text>Estado: {scannedDataID.Estado}</Text>
            <Text>Tipo de red: {scannedDataID.Tipo_de_red}</Text>
  ...
  function DatosNoEncontrados() {
    return (
      <View style={styles.container}>
        <Text style={styles.title}>Datos no encontrados</Text>
        <Button title="Escanear de nuevo" onPress={handleScanAgain} />
      </View>
    );
  }
...
const styles = StyleSheet.create({
  ...
  buttonsContainer: {
    position: "absolute",
    top: 20, //
    left: 200, // Ajusta este valor para moverlo hacia la izquierda
  },
